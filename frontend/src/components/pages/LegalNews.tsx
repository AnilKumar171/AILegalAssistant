            ...
            {(selectedCategory === 'india' ? isLoading : isLoadingGlobal) ? (
              ...
              <div className="space-y-6">
                {filterArticles().map((article, index) => (
                  <article
                    key={index}
                    className="group bg-white rounded-2xl shadow-sm border border-slate-200 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 overflow-hidden"
                  >
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-0">
                      <div className="md:col-span-2 relative">
                        <img
                          src={article.image || 'https://images.unsplash.com/photo-1589395937658-0557e7d9893f?q=80&w=1200&auto=format&fit=crop'}
                          alt={article.title}
                          className="w-full h-56 md:h-full object-cover"
                          loading="lazy"
                        />
                        <div className="absolute top-3 left-3 flex items-center gap-2 bg-white/90 backdrop-blur px-2 py-1 rounded-lg text-xs font-medium shadow">
                          <img src={getFavicon(article.url)} alt="src" className="w-4 h-4" />
                          <span className="text-slate-700">{article.source.name}</span>
                        </div>
                      </div>
                      <div className="md:col-span-3 p-5 md:p-6">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span className="flex items-center gap-1 text-xs text-slate-500 whitespace-nowrap bg-slate-100 px-2 py-1 rounded-full">
                        ...
                        <div className="mt-4 flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <span className="text-xs text-blue-700 bg-blue-50 px-2 py-1 rounded-full font-semibold">Legal</span>
                            {selectedCategory === 'india' ? (
                              <span className="text-xs text-emerald-700 bg-emerald-50 px-2 py-1 rounded-full font-semibold">India</span>
                            ) : (
                              <span className="text-xs text-purple-700 bg-purple-50 px-2 py-1 rounded-full font-semibold">Global</span>
                            )}
                          </div>
                          <a
                            href={article.url}
